            ...
            if (appointment.status === "pending") {
                actionButtonsHTML = `
                    <div style="display:flex; gap:6px; flex-wrap:nowrap;">
                        <button class="row-action-button row-action-button--approve"  onclick="openapprovedationModal(${appointment.id}, 'approved')">Confirm</button>
                        <button class="row-action-button row-action-button--cancel"   onclick="openapprovedationModal(${appointment.id}, 'cancel')">Cancel</button>
                    </div>`;
            } else if (appointment.status === "approved") {
                actionButtonsHTML = `
                    <div style="display:flex; gap:4px; flex-wrap:none;">
                        <button class="row-action-button row-action-button--complete" onclick="openapprovedationModal(${appointment.id}, 'complete')">Complete</button>
                        <button class="row-action-button row-action-button--cancel"   onclick="openapprovedationModal(${appointment.id}, 'cancel')">Cancel</button>
                    </div>`;
            } else {
...
function openapprovedationModal(appointmentId, actionType) {
...
async function doAction() {
    ...
    try {
        ...
        const toastMessageMap = {
            approved: `Appointment approved for ${appointment.patient}`,
